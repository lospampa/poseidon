...
void lib_end_parallel_region(){
	...
	char set[2];
        if(libKernels[id_actual_region].state !=END){
                ...
                switch(libKernels[id_actual_region].state){
                        case REPEAT:
                                libKernels[id_actual_region].numThreads = libKernels[id_actual_region].numCores;
				libKernels[id_actual_region].state = S0;
                        ...
                        case S1:
                                if(result < libKernels[id_actual_region].bestResult){ //comparing S0 to REPEAT
                                        libKernels[id_actual_region].bestResult = result;
                                        libKernels[id_actual_region].bestThread = libKernels[id_actual_region].numThreads;
					if(libKernels[id_actual_region].numThreads / 2 >= 2){
						...
						libKernels[id_actual_region].numThreads = libKernels[id_actual_region].numThreads + libKernel[id_actual_region].pass;
						if(libKernels[id_actual_region].pass == 1)
							libKernels[id_actual_region].state = S3;
						else
							libKernels[id_actual_region].state = S2;
                                }else{ 
					if(libKernels[id_actual_region].bestThread == libKernels[id_actual_region].numCores){
