...
void lib_init(int metric, int start_search){
	...
	while(startThreads != 2 && startThreads != 3 && startThreads != 5){
		...
		libKernels[i].bestFreq = TURBO_ON;
                ...
                libKernels[i].lastBestFreq = TURBO_OFF;
                libKernels[i].lastBestFreqSeq = TURBO_OFF;
...
int lib_resolve_num_threads(uintptr_t ptr_region){
	...
	char set[2];
        ...
        switch(libKernels[id_actual_region].state){
			...
			break;
		case END_THREADS:
			if(libKernels[id_actual_region].bestFreq != libKernels[id_previous_region].lastBestFreqSeq && libKernels[id_actual_region].bestTime > write_file_threshold){ //0.1
	...
	char set[2];
        ...
        switch(libKernels[id_actual_region].seqState){
			...
			libKernels[id_actual_region].seqState = INITIAL;
                        if (libKernels[id_actual_region].bestFreqSeq != libKernels[id_actual_region].lastBestFreq){
                		libKernels[id_actual_region].lastBestFreqSeq = libKernels[id_actual_region].bestFreqSeq;
                                fd = open("/sys/devices/system/cpu/cpufreq/boost", O_WRONLY);
			...
			initSeqTime = omp_get_wtime();
                        ...
                        break;
		case END_TURBO:
			if (libKernels[id_actual_region].bestFreqSeq != libKernels[id_actual_region].lastBestFreq && write_file_threshold < libKernels[id_actual_region].timeSeqTurboOn){
                		libKernels[id_actual_region].lastBestFreqSeq = libKernels[id_actual_region].bestFreqSeq;
                                fd = open("/sys/devices/system/cpu/cpufreq/boost", O_WRONLY);
        			sprintf(set, "%d", libKernels[id_actual_region].bestFreqSeq);
	        		write(fd, set, sizeof(set));
