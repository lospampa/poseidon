...
/* First function called. It initiallizes all the functions and variables used by Poseidon */
...
void lib_end_parallel_region(){
	...
	switch(libKernels[id_actual_region].seqState){
			...
			break;
		case END_TURBO:
			if (libKernels[id_actual_region].bestFreqSeq != libKernels[id_actual_region].lastBestFreq && libKernels[id_actual_region].timeSeqTurboOff > write_file_threshold){
                		libKernels[id_actual_region].lastBestFreqSeq = libKernels[id_actual_region].bestFreqSeq;
				fd = open("/sys/devices/system/cpu/cpufreq/boost", O_WRONLY);
        			sprintf(set, "%d", libKernels[id_actual_region].bestFreqSeq);
	        		write(fd, set, sizeof(set));
...
/* It finalizes the environment of Poseidon */
void lib_destructor()
{
